...
R8通知PDF（令和８年保医発0305第６号）とR6通知PDF（令和６年保医発0305第４号）を比較し、
...
R6_PDF_PATH = os.path.join(SCRIPT_DIR, "..", "令和6年度診療報酬", "原本",
    "診療報酬の算定方法の一部改正に伴う実施上の留意事項について（通知） 令和６年３月５日保医発0305第４号 別添１ （医科点数表）.pdf")
...
RE_HEADING_ONLY = re.compile(
    r'^(第[１２３４５６７８９０\d]+\s*[章部節款]\s+.*|区分|削除\s*|[＜<]?通則[＞>]?|別添.*|＜.*＞)$')
# カナ項目検出
...
def main():
    ...
    print("通知PDF新旧対照表 抽出開始", file=sys.stderr)
    print("=" * 60, file=sys.stderr)
